...
//
// libslic3r integration by tamasmeszaros
...
template<> struct vertex_traits<Slic3r::Vec3f> {
    using coord_type = float;
    using compute_type = double;
    
    static inline float x(const Slic3r::Vec3f &v) { return v.x(); }
    static inline float& x(Slic3r::Vec3f &v) { return v.x(); }
    
    static inline float y(const Slic3r::Vec3f &v) { return v.y(); }
    static inline float& y(Slic3r::Vec3f &v) { return v.y(); }
    
    static inline float z(const Slic3r::Vec3f &v) { return v.z(); }
    static inline float& z(Slic3r::Vec3f &v) { return v.z(); }
};

template<> struct mesh_traits<Slic3r::TriangleMesh> {
    using vertex_t = Slic3r::Vec3f;
    static size_t face_count(const Slic3r::TriangleMesh &m) { return m.its.indices.size(); }
    static size_t vertex_count(const Slic3r::TriangleMesh &m) { return m.its.vertices.size(); }
    static vertex_t vertex(const Slic3r::TriangleMesh &m, size_t idx) { return m.its.vertices[idx];}
    static std::array<size_t, 3> triangle(const Slic3r::TriangleMesh &m, size_t idx)
    {
        std::array<size_t, 3> t;
        for (size_t i = 0; i < 3; ++i) t[i] = size_t(m.its.indices[idx](int(i)));
        return t;
    }
};
...
void simplify_mesh(TriangleMesh &m)
{
    SimplifyMesh::implementation::SimplifiableMesh<TriangleMesh> sm(&m);
}
